...
class BaseOptions(object):
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--img_root', type=str, default='datasets/', help='path to the original images.')
        self.parser.add_argument('--sources', type=str, nargs = '+', default=['cuhk03','viper','ilids','3dpes'], help='source domain list')
        self.parser.add_argument('--target', type=str, nargs=1, default='market', help='path to the original images.')
        self.parser.add_argument('--batch_size', type=int, default=16)
        self.parser.add_argument('--seed', type=int, default=1)
        self.parser.add_argument('--print_freq', default=500, type=int, help='print after several batches')
        ...
        self.parser.add_argument('--num_share_block', type=int, default=1, help='Number of ResNet50 blocks to be shared')
        self.parser.add_argument('--evaluate_freq', type=int, default=10, help='interval to evaluate')
        self.parser.add_argument('--record_freq', type=int, default=5, help='interval to evaluate')
        self.parser.add_argument('--method', type=str, default='CDA_loss', help='name_of_method')
        self.parser.add_argument('--market_root', type=str, default='/home/shuhan/ReID_data/DUKE/ReID_IDE_pytorch-master/data/', help='root of Market and Duke')
        self.parser.add_argument('--preFeaNet', type=int, default=1, help='to resume from pretrained joint_tune model')
        self.parser.add_argument('--lockShare', type=int, default=1, help='whether to lock shared feature net')
        self.parser.add_argument('--domainID', type=int, default=-1, help='to only use split net trained with domainID, -1 for using all')
...
class Recorder(object):
    ...
    def update(self, epoch, is_train, meters):
        ...
        if is_train:
            ...
            int_epochs = epoch - last_epoch
            for stat, meter in meters.items():
                last_meter = self.stat[stat][last_epoch, 0]
                cur_meter = float(meter.avg)
                int_meter = (cur_meter - last_meter) / int_epochs
                for i in range(int_epochs):
                    self.stat[stat][last_epoch + i + 1, 0] = last_meter + int_meter * (i + 1)
            return False
        else:
            last_epoch = self.current_epoch_test
            self.current_epoch_test = epoch
            int_epochs = epoch - last_epoch
            for stat, meter in meters.items():
                last_meter = self.stat[stat][last_epoch, 1]
                cur_meter = float(meter.avg)
                int_meter = (cur_meter - last_meter) / int_epochs
                for i in range(int_epochs):
                    self.stat[stat][last_epoch + i + 1, 1] = last_meter + int_meter * (i + 1)
            assert self.metric in meters.keys()
            metric_val = meters[self.metric].avg
            _, metric_highest = self.retrieve_stat(self.metric, 'max')
            return metric_val.item() == metric_highest
...
def extract_features(loader, model, index_feature=0, require_views=True):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(loader):
            ...
            domain_prediction = output_tuple[1].data.cpu().numpy()
            domain_predictions.append(np.mean(domain_prediction, axis=0).tolist())
        ...
        avg_domain_prediciton = np.mean(domain_predictions, axis=0)
        print('Domain Prediction:')
        print(avg_domain_prediciton.tolist())
...
def extract_split(loader, nets, domains, index_feature=0, require_views=True):
    labels = torch.zeros((len(loader.dataset),))
    views = torch.zeros((len(loader.dataset),))
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(loader):
            ...
            img_features = nets['sharedNet'](imgs_var)
            _, domPred = nets['dClassNet'](img_features)
            m = torch.nn.Softmax()
            domPred = m(domPred, dim=0)
            feature = torch.zeros([batch_size, 2048]).cuda()

            for idx, domain in enumerate(domains):
                domScore = torch.tensor([[domPred[i][idx]] for i in range(batch_size)])
                domScore = domScore.repeat(1, 2048)
                domFea, _ = nets[domain](img_features)
                feature += domScore.cuda() * domFea
...
def extract_features_id(loader, model, domainID, index_feature=0, require_views=True, ):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(loader):
            ...
            output_tuple = model(imgs_var, domainID)
            feature_batch = output_tuple[index_feature]
            feature_batch = feature_batch.data.cpu()
