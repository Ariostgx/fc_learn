...
from utils import AverageMeter, BaseOptions, Recorder, Logger, time_string, convert_secs2time, eval_cmc_map, \
    reset_state_dict, extract_features_id, create_stat_string, save_checkpoint, adjust_learning_rate, accuracy, \
    partition_params
import dataloader
import img_root_txt
...
def load_pretrain(model):
    ...
    for key, value in model.splitNets.items():
        splitNetOne = value
        break
    model_dict = splitNetOne['feaNet'].state_dict()
    for k, _ in model_dict.items():
        model_dict[k] = pretrained_dict[k]
    for domain in model.splitNets.keys():
        model.splitNets[domain]['feaNet'].load_state_dict(model_dict)
...
def load_resume(net, state_dict):
    ...
    for domain in net.splitNets.keys():
        net.splitNets[domain]['feaNet'].load_state_dict(state_dict[str(domain)]['feaNet'])
        net.splitNets[domain]['cls_fc'].load_state_dict(state_dict[str(domain)]['cls_fc'])
...
def main():
    ...
    net = resnet_mmd.splitNet(class_numbers, domains, args.num_share_block)

    net = net.cuda()
    for domain, subnet in net.splitNets.items():
        subnet['feaNet'] = subnet['feaNet'].cuda()
        subnet['cls_fc'] = subnet['cls_fc'].cuda()
    load_pretrain(net)
    ...
    init_lr = []
    parameters = [{'params': net.sharedNet.parameters(), 'lr': 0}]
    init_lr.append(0)
    parameters.append({'params': net.domain_feaNet.parameters(), 'lr': args.lr / 10})
    init_lr.append(args.lr / 10)
    parameters.append({'params': net.domain_cls.parameters(), 'lr': args.lr})
    init_lr.append(args.lr)
    for domain in domains:
        parameters.append({'params': net.splitNets[domain]['feaNet'].parameters(), 'lr': args.lr / 10})
        init_lr.append(args.lr / 10)
        parameters.append({'params': net.splitNets[domain]['cls_fc'].parameters(), 'lr': args.lr})
        init_lr.append(args.lr)

    optimizer = torch.optim.SGD(parameters, lr=0, momentum=args.momentum, weight_decay=args.wd)
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        lr = adjust_learning_rate(optimizer, init_lr, epoch - 1, args.epochs, args.lr_strategy)
        ...
        if epoch % args.record_freq == 0:
            ...
            for domain, subnet in net.splitNets.items():
                checkpoint['state_dict'][str(domain)] = {}
                checkpoint['state_dict'][str(domain)]['feaNet'] = subnet['feaNet'].state_dict()
                checkpoint['state_dict'][str(domain)]['cls_fc'] = subnet['cls_fc'].state_dict()
        ...
        if epoch % args.evaluate_freq == 0 or epoch == 1:
            evaluate(target_gallery_loader, target_prob_loader, net,
                     epoch, recorder, logger, args.domainID)
...
def evaluate(gallery_loader, probe_loader, net,
             epoch, recorder, logger, domainID):
    stats = recorder.val_stats
    meters = {stat: AverageMeter() for stat in stats}
    net.eval()

    gallery_features, gallery_labels, gallery_views = extract_features_id(gallery_loader, net, domainID, index_feature=0,
                                                                       require_views=True)
    probe_features, probe_labels, probe_views = extract_features_id(probe_loader, net, domainID, index_feature=0, require_views=True)
